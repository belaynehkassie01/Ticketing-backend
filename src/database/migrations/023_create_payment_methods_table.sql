...
-- Ethiopian Context: Local payment methods, ETB currency, Ethiopian bank integration
-- ============================================

CREATE TABLE IF NOT EXISTS `payment_methods` (
  -- Primary identifier
  `id` BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT 'Internal payment method ID',
  ...
  -- Classification
  `type` ENUM('mobile_money', 'bank_transfer', 'card', 'cash', 'wallet') NOT NULL 
    COMMENT 'Payment type (mobile_money for Telebirr, bank_transfer for CBE)',
  
  -- Status & Display
  `is_active` BOOLEAN DEFAULT TRUE COMMENT 'Is this payment method currently available?',
  `is_default` BOOLEAN DEFAULT FALSE COMMENT 'Should this be the default selection?',
  `sort_order` INT DEFAULT 0 COMMENT 'Display order (0 = first)',
  ...
  `min_amount` DECIMAL(10,2) DEFAULT 1.00 COMMENT 'Minimum payment amount in ETB',
  `max_amount` DECIMAL(15,2) DEFAULT 1000000.00 COMMENT 'Maximum payment amount in ETB',
  ...
  `swift_code` VARCHAR(11) NULL COMMENT 'SWIFT/BIC code for international',
  
  -- QR Code Support (Ethiopian Mobile Payments)
  `qr_supported` BOOLEAN DEFAULT FALSE COMMENT 'Does this method support QR payments?',
  ...
  `has_fee` BOOLEAN DEFAULT FALSE COMMENT 'Does this method have transaction fees?',
  `fee_type` ENUM('percentage', 'fixed', 'both', 'none') DEFAULT 'none',
  `fee_percentage` DECIMAL(5,2) DEFAULT 0.00 COMMENT 'Percentage fee (e.g., 1.5%)',
  `fee_fixed` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Fixed fee in ETB',
  `fee_capped_at` DECIMAL(10,2) NULL COMMENT 'Maximum fee amount in ETB',
  `who_pays_fee` ENUM('customer', 'platform', 'organizer', 'split') DEFAULT 'customer',
  
  -- API & Integration Configuration
  `api_config` JSON COMMENT 'Payment gateway configuration (API keys, endpoints, etc.)',
  ...
  `api_version` VARCHAR(20) DEFAULT 'v1' COMMENT 'API version for integration',
  ...
  `requires_verification` BOOLEAN DEFAULT FALSE COMMENT 'Requires manual verification (e.g., CBE transfers)',
  `verification_time_minutes` INT DEFAULT 60 COMMENT 'Estimated verification time in minutes',
  `supports_refunds` BOOLEAN DEFAULT TRUE COMMENT 'Does this method support refunds?',
  `refund_time_hours` INT DEFAULT 24 COMMENT 'Estimated refund processing time in hours',
  ...
  -- Performance & Metrics
  `success_rate` DECIMAL(5,2) DEFAULT 100.00 COMMENT 'Historical success rate percentage',
  `avg_processing_seconds` INT DEFAULT 60 COMMENT 'Average processing time in seconds',
  `total_transactions` BIGINT UNSIGNED DEFAULT 0 COMMENT 'Total transactions processed',
  `total_volume` DECIMAL(20,2) DEFAULT 0.00 COMMENT 'Total transaction volume in ETB',
  
  -- Security & Fraud Prevention
  `supports_3d_secure` BOOLEAN DEFAULT FALSE COMMENT 'Supports 3D Secure authentication',
  `fraud_score_threshold` DECIMAL(5,2) DEFAULT 80.00 COMMENT 'Fraud score threshold for auto-rejection',
  `requires_otp` BOOLEAN DEFAULT TRUE COMMENT 'Requires OTP verification',
  
  -- Timestamps with soft delete support
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `activated_at` DATETIME NULL COMMENT 'When this payment method was activated',
  `deactivated_at` DATETIME NULL COMMENT 'When this payment method was deactivated',
  ...
  INDEX `idx_available_for_country` (`available_for_country`),
  INDEX `idx_requires_verification` (`requires_verification`),
  INDEX `idx_environment` (`environment`),
  INDEX `idx_deleted_at` (`deleted_at`),
  
  -- Unique constraints
  UNIQUE INDEX `uq_code` (`code`),
  UNIQUE INDEX `uq_name_country` (`name`, `available_for_country`, `deleted_at`),
  ...
  CONSTRAINT `chk_processing_time` CHECK (`avg_processing_seconds` >= 0),
  CONSTRAINT `chk_currency_etb` CHECK (`currency` = 'ETB'), -- Ethiopian-only for now
  CONSTRAINT `chk_country_et` CHECK (`available_for_country` = 'ET'), -- Ethiopia only
  CONSTRAINT `chk_fee_capped` CHECK (`fee_capped_at` IS NULL OR `fee_capped_at` >= 0)
...
-- Trigger 3: Update activation/deactivation timestamps (FIXED)
...
BEGIN
  ...
  IF NEW.is_active = FALSE AND OLD.is_active = TRUE THEN
    SET NEW.deactivated_at = CURRENT_TIMESTAMP;
  END IF;
...
CREATE PROCEDURE `sp_calculate_payment_fees_et`(
  ...
  OUT p_gross_amount DECIMAL(15,2)
)
BEGIN
  DECLARE v_fee_percentage DECIMAL(5,2);
  DECLARE v_fee_fixed DECIMAL(10,2);
  DECLARE v_fee_capped_at DECIMAL(10,2);
  DECLARE v_fee_type ENUM('percentage', 'fixed', 'both', 'none');
