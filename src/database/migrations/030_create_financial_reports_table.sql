...
CREATE TABLE IF NOT EXISTS `financial_reports` (
  ...
  `ethiopian_month_number` TINYINT NULL COMMENT '1-13 (application calculated)',
  ...
  `total_organizers_count` INT DEFAULT 0,
  
  -- Revenue breakdown
  `ticket_sales_revenue` DECIMAL(15,2) DEFAULT 0.00,
  `platform_commission_revenue` DECIMAL(15,2) DEFAULT 0.00,
  `service_fee_revenue` DECIMAL(15,2) DEFAULT 0.00,
  `other_revenue` DECIMAL(15,2) DEFAULT 0.00,
  
  -- Ethiopian VAT tracking (15%)
  `total_vat_collected` DECIMAL(15,2) DEFAULT 0.00,
  `vat_on_ticket_sales` DECIMAL(15,2) DEFAULT 0.00,
  `vat_on_commission` DECIMAL(15,2) DEFAULT 0.00,
  `vat_payable_to_erca` DECIMAL(15,2) DEFAULT 0.00,
  `vat_return_submitted` BOOLEAN DEFAULT FALSE,
  `vat_return_date` DATE NULL,
  `vat_return_reference` VARCHAR(100) NULL,
  
  -- Withholding tax (2%)
  `total_withholding_tax` DECIMAL(15,2) DEFAULT 0.00,
  `withholding_on_payouts` DECIMAL(15,2) DEFAULT 0.00,
  `withholding_return_submitted` BOOLEAN DEFAULT FALSE,
  `withholding_return_date` DATE NULL,
  
  -- Payouts
  `total_payouts_issued` DECIMAL(15,2) DEFAULT 0.00,
  `payouts_count` INT DEFAULT 0,
  `pending_payouts` DECIMAL(15,2) DEFAULT 0.00,
  
  -- Commission
  `total_platform_commission` DECIMAL(15,2) DEFAULT 0.00,
  `average_commission_rate` DECIMAL(5,2) DEFAULT 10.00,
  
  -- Payment methods
  `telebirr_revenue` DECIMAL(15,2) DEFAULT 0.00,
  `cbe_bank_transfer_revenue` DECIMAL(15,2) DEFAULT 0.00,
  `other_payment_revenue` DECIMAL(15,2) DEFAULT 0.00,
  ...
  -- Growth metrics
  `revenue_growth_percentage` DECIMAL(5,2) DEFAULT 0.00,
  `ticket_sales_growth` DECIMAL(5,2) DEFAULT 0.00,
  `organizer_growth` DECIMAL(5,2) DEFAULT 0.00,
...
CREATE PROCEDURE `sp_generate_monthly_financial_report_et`(
  IN p_year INT,
  IN p_month INT,
  IN p_generated_by BIGINT UNSIGNED,
  OUT p_report_id BIGINT UNSIGNED,
...
BEGIN
  ...
  DECLARE v_total_vat DECIMAL(15,2);
  DECLARE v_total_tickets INT;
  DECLARE v_existing_report_id BIGINT UNSIGNED;
  ...
  -- GET report code
  SELECT `report_code` INTO p_report_code
  FROM `financial_reports`
...
BEGIN
  ...
  DECLARE v_fiscal_year INT;
  DECLARE v_vat_on_sales DECIMAL(15,2);
  DECLARE v_vat_on_purchases DECIMAL(15,2);
  DECLARE v_net_vat_payable DECIMAL(15,2);
