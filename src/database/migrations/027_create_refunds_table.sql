...
--          Telebirr/CBE processing, and integration with disputes
-- Dependencies: 
...
-- 2. information_schema.TABLES.AUTO_INCREMENT FIXED → UUID-based reference
-- 3. DO SLEEP(2) REMOVED → No blocking delays
-- 4. ticket_number NOT NULL FIXED → NULLABLE (handled by trigger)
-- 5. Floating point CHECK FIXED → ROUND() for precision
-- 6. Unique constraint logic FIXED → Better validation in trigger
-- ============================================

CREATE TABLE IF NOT EXISTS `refunds` (
  -- Primary identifier
  `id` BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT 'Internal refund ID',
  ...
  `ticket_id` BIGINT UNSIGNED NULL COMMENT 'References individual_tickets.id (nullable for bulk refunds)',
  `ticket_number` VARCHAR(50) NULL COMMENT 'Denormalized copy for quick lookup (NULLABLE - FIXED)',
  
  `user_id` BIGINT UNSIGNED NOT NULL COMMENT 'References users.id (refund recipient)',
  ...
  `original_payment_amount` DECIMAL(15,2) NOT NULL COMMENT 'Original payment amount (ETB)',
  `original_vat_amount` DECIMAL(15,2) NOT NULL COMMENT 'Original VAT amount (ETB)',
  `original_platform_commission` DECIMAL(15,2) NOT NULL COMMENT 'Original commission (ETB)',
  
  `refund_amount` DECIMAL(15,2) NOT NULL COMMENT 'Amount to refund (ETB)',
  `refund_currency` CHAR(3) DEFAULT 'ETB' NOT NULL COMMENT 'Refund currency',
  `refund_vat_amount` DECIMAL(15,2) NOT NULL COMMENT 'VAT portion of refund',
  `refund_commission_amount` DECIMAL(15,2) NOT NULL COMMENT 'Commission to refund',
  `net_refund_amount` DECIMAL(15,2) NOT NULL COMMENT 'Net amount after commission',
  ...
  `refund_reason` ENUM(
    ...
    'quality_issue',
    'other'
  ) NOT NULL COMMENT 'Reason for refund',
  ...
  `refund_percentage` DECIMAL(5,2) NULL COMMENT 'Percentage of original amount refunded',
  ...
  `status` ENUM(
    ...
    'failed',
    'cancelled'
  ) DEFAULT 'draft' COMMENT 'Refund status',
  
  `approval_level` ENUM('organizer', 'admin', 'automatic') DEFAULT 'admin' COMMENT 'Who must approve',
  `requires_approval` BOOLEAN DEFAULT TRUE COMMENT 'Whether approval is required',
  
  -- Request Details
  `requested_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'When refund was requested',
  `requested_by` BIGINT UNSIGNED NOT NULL COMMENT 'References users.id (who requested)',
  `request_method` ENUM('customer_portal', 'organizer_portal', 'admin_portal', 'api', 'support_ticket') 
    DEFAULT 'customer_portal' COMMENT 'How refund was requested',
  
  -- Approval Details
  `approved_at` DATETIME NULL COMMENT 'When refund was approved',
  `approved_by` BIGINT UNSIGNED NULL COMMENT 'References users.id (who approved)',
  `approval_notes` TEXT COMMENT 'Notes from approver',
  
  `rejected_at` DATETIME NULL COMMENT 'When refund was rejected',
  `rejected_by` BIGINT UNSIGNED NULL COMMENT 'References users.id (who rejected)',
  ...
  `processing_method` ENUM('original_method', 'telebirr', 'cbe_transfer', 'bank_transfer', 'wallet_credit', 'manual_cash') 
  ...
  `commission_refunded` BOOLEAN DEFAULT FALSE COMMENT 'Whether commission was refunded',
  `commission_refund_amount` DECIMAL(15,2) DEFAULT 0.00 COMMENT 'Commission amount refunded',
  `commission_refund_percentage` DECIMAL(5,2) NULL COMMENT 'Percentage of commission refunded',
  
  `organizer_liable` BOOLEAN DEFAULT FALSE COMMENT 'Whether organizer bears cost',
  `organizer_contribution` DECIMAL(15,2) DEFAULT 0.00 COMMENT 'Amount organizer contributes',
  
  -- Dispute Integration (ETHIOPIAN SUPPORT WORKFLOW)
  `dispute_id` BIGINT UNSIGNED NULL COMMENT 'References disputes.id',
  `dispute_reference` VARCHAR(100) NULL COMMENT 'Denormalized dispute reference',
  `dispute_resolution` ENUM('full_refund', 'partial_refund', 'no_refund', 'credit', 'other') NULL,
  ...
  `tax_implications` BOOLEAN DEFAULT FALSE COMMENT 'Whether refund has tax implications',
  `vat_adjustment_required` BOOLEAN DEFAULT FALSE COMMENT 'Whether VAT needs adjustment',
  `compliance_notes` TEXT COMMENT 'Compliance and regulatory notes',
  
  -- Communication Log
  `customer_notified_at` DATETIME NULL COMMENT 'When customer was notified',
  `customer_notification_method` ENUM('sms', 'email', 'in_app', 'whatsapp', 'phone') NULL,
  `organizer_notified_at` DATETIME NULL COMMENT 'When organizer was notified',
  
  -- Timestamps
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `processed_at` DATETIME NULL COMMENT 'When refund processing started',
  `completed_at` DATETIME NULL COMMENT 'When refund was completed',
  `failed_at` DATETIME NULL COMMENT 'When refund failed',
  ...
  INDEX `idx_event_status` (`event_id`, `status`, `requested_at`),
  INDEX `idx_status_date` (`status`, `requested_at`),
  INDEX `idx_refund_reason` (`refund_reason`, `status`),
  INDEX `idx_processing_method` (`processing_method`, `status`),
  INDEX `idx_deleted_at` (`deleted_at`),
  
  -- Unique constraints (FIXED - safe for concurrency)
  UNIQUE INDEX `uq_refund_reference` (`refund_reference`) COMMENT 'Unique refund reference',
  UNIQUE INDEX `uq_external_transaction` (`refund_transaction_id`) COMMENT 'Unique external transaction ID',
  
  -- Business Logic Constraints (FIXED - with ROUND for floating point precision)
  CONSTRAINT `chk_refund_amount` CHECK (`refund_amount` > 0),
  CONSTRAINT `chk_refund_not_exceed_original` CHECK (`refund_amount` <= `original_payment_amount`),
  CONSTRAINT `chk_currency_etb` CHECK (`refund_currency` = 'ETB'),
  CONSTRAINT `chk_vat_amount` CHECK (`refund_vat_amount` >= 0),
  CONSTRAINT `chk_commission_refund` CHECK (`commission_refund_amount` >= 0),
  CONSTRAINT `chk_organizer_contribution` CHECK (`organizer_contribution` >= 0),
  ...
  COMMENT='Ethiopian refund management with multi-level approval, commission handling, and integration with disputes. FIXED VERSION - production ready.';
...
BEGIN
  ...
  DECLARE v_payment_amount DECIMAL(15,2);
  ...
  IF NEW.dispute_id IS NOT NULL THEN
    SELECT `dispute_reference` INTO v_dispute_reference
    FROM `disputes`
...
-- Trigger 2: Handle status transitions and timestamps (FIXED - no heavy business logic)
...
BEGIN
  ...
  -- Handle status transitions and set timestamps (lightweight)
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      WHEN e.`start_date` > DATE_ADD(NOW(), INTERVAL 7 DAY) THEN 'full_refund_7_days'
      WHEN e.`start_date` > DATE_ADD(NOW(), INTERVAL 3 DAY) THEN 'partial_refund_50_percent'
      WHEN e.`start_date` > NOW() THEN 'partial_refund_25_percent'
      ELSE 'no_refund_after_event'
    END,
    CASE
      WHEN e.`start_date` > DATE_ADD(NOW(), INTERVAL 7 DAY) THEN 100.00
      WHEN e.`start_date` > DATE_ADD(NOW(), INTERVAL 3 DAY) THEN 50.00
  ...
  SELECT `refund_reference` INTO p_refund_reference
  FROM `refunds`
...
CREATE PROCEDURE `sp_process_refund_decision_et`(
  ...
  IN p_commission_refund_percentage DECIMAL(5,2),
  IN p_organizer_contribution DECIMAL(15,2),
  OUT p_success BOOLEAN,
  OUT p_message VARCHAR(500)
)
BEGIN
  DECLARE v_current_status VARCHAR(30);
  DECLARE v_refund_amount DECIMAL(15,2);
  DECLARE v_original_commission DECIMAL(15,2);
  DECLARE v_approval_level VARCHAR(20);
  DECLARE v_organizer_id BIGINT UNSIGNED;
  ...
  IF p_decision = 'approve' THEN
    ...
    DECLARE v_commission_refund_amount DECIMAL(15,2);
