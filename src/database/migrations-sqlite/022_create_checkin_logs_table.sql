...
-- Original file: 022_create_checkin_logs_table.sql
...
CREATE TABLE IF NOT EXISTS checkin_logs (
  -- Primary identifier
  id INTEGEREGER PRIMARY KEY AUTOINCREMENT COMMENT 'INTEGERernal check-in log ID',
  
  -- Ticket & Event Relationships
  ticket_id INTEGEREGER NOT NULL COMMENT 'References individual_tickets.id',
  ...
  -- Check-in Personnel
  checked_in_by INTEGEREGER NOT NULL COMMENT 'References users.id (staff who performed check-in)',
  ...
  -- TEXTs
  checkin_time TEXT NOT NULL COMMENT 'Actual check-in time (Ethiopian timezone aware)',
  server_time TEXT DEFAULT CURRENT_TEXT COMMENT 'When record reached server',
  ...
  latitude REAL NULL COMMENT 'GPS latitude (Addis Ababa ≈ 9.032, 38.746)',
  longitude REAL NULL COMMENT 'GPS longitude',
  location_poINTEGER POINTEGER GENERATED ALWAYS AS (
    IF(latitude IS NOT NULL AND longitude IS NOT NULL,
       ST_SRID(POINTEGER(longitude, latitude), 4326),
       NULL)
  ) STORED COMMENT 'Spatial poINTEGER for geographical queries (WGS 84)',
  location_name VARCHAR(255) COMMENT 'Human-readable location (e.g., "Millennium Hall Main Gate")',
  city_id INTEGEREGER NULL COMMENT 'References cities.id for Ethiopian city',
  
  -- Network & Connectivity (Ethiopian Infrastructure)
  is_online INTEGER DEFAULT TRUE COMMENT 'Was device online during check-in?',
  connection_type TEXT DEFAULT 'unknown'
    COMMENT 'Network type during check-in (important for Ethiopian connectivity analysis)',
  network_speed_kbps INTEGER NULL COMMENT 'Network speed in kbps (for performance monitoring)',
  ...
  synced_at TEXT NULL COMMENT 'When offline data was synced to server',
  
  -- Validation & Security
  is_valid INTEGER DEFAULT TRUE COMMENT 'Was this a valid check-in?',
  validation_notes TEXT COMMENT 'Notes on validation (e.g., "Ticket already checked in at 14:30")',
  qr_scan_duration_ms INTEGER NULL COMMENT 'How long QR scan took in milliseconds',
  ...
  checkin_hour INTEGEREGER GENERATED ALWAYS AS (HOUR(checkin_time)) STORED 
    COMMENT 'Hour of check-in (24-hour format) for Ethiopian event timing analysis',
  checkin_day_of_week INTEGEREGER GENERATED ALWAYS AS (DAYOFWEEK(checkin_time)) STORED 
  ...
  INDEX idx_ticket (ticket_id), -- INDEX converted separately (event_id), -- INDEX converted separately (organizer_id), -- INDEX converted separately (checkin_time), -- INDEX converted separately (sync_status), -- INDEX converted separately (checked_in_by), -- INDEX converted separately (device_id),
  
  -- Composite indexes for common Ethiopian queries
  INDEX idx_event_checkin_time (event_id, checkin_time), -- INDEX converted separately (organizer_id, checkin_time), -- INDEX converted separately (ticket_id, checkin_time), -- INDEX converted separately (sync_status, synced_at), -- INDEX converted separately (checkin_method, checkin_time), -- INDEX converted separately (connection_type, checkin_time),
  
  -- Spatial index for Ethiopian location queries (MySQL-compatible)
  SPATIAL INDEX idx_location (location_poINTEGER),
  
  -- Unique constraINTEGER: Only ONE valid check-in per ticket
  ...
  CONSTRAINTEGER chk_network_speed CHECK (
    network_speed_kbps IS NULL OR network_speed_kbps >= 0
  ),
  
  CONSTRAINTEGER chk_scan_duration CHECK (
    qr_scan_duration_ms IS NULL OR qr_scan_duration_ms >= 0
  ),
  
  CONSTRAINTEGER chk_ethiopian_hours CHECK (
...
BEGIN
  ...
  IF NEW.server_time IS NULL THEN
    SET NEW.server_time = CURRENT_TEXT;
  END IF;
...
FOR EACH ROW
BEGIN
  DECLARE existing_valid_checkins INTEGER;
  DECLARE ticket_status VARCHAR(50);
  
  -- Check if ticket already has a valid check-in
  SELECT COUNT(*) INTEGERO existing_valid_checkins
  FROM checkin_logs
  WHERE ticket_id = NEW.ticket_id 
    AND is_valid = TRUE;
  
  -- Get ticket status
  SELECT status INTEGERO ticket_status
...
BEGIN
  ...
  IF OLD.sync_status = 'pending' AND NEW.sync_status = 'synced' THEN
    SET NEW.synced_at = CURRENT_TEXT;
  END IF;
...
-- ALTER TABLE events ADD COLUMN IF NOT EXISTS checkins_count INTEGER DEFAULT 0 COMMENT 'Total valid check-ins for this event';