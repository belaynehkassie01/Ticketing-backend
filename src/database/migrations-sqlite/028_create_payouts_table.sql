...
-- Original file: 028_create_payouts_table.sql
...
-- Purpose: Store organizer payout records with Ethiopian banking INTEGERegration, tax compliance,
--          and proper MySQL constraINTEGER handling
...
CREATE TABLE IF NOT EXISTS payouts (
  -- Primary identifier
  id INTEGEREGER PRIMARY KEY AUTOINCREMENT COMMENT 'INTEGERernal payout ID',
  ...
  user_id INTEGEREGER NOT NULL COMMENT 'References users.id (organizer account)',
  
  -- Financial amounts (ETB only)
  amount REAL NOT NULL COMMENT 'Gross payout amount before deductions',
  currency CHAR(3) DEFAULT 'ETB' NOT NULL COMMENT 'Always ETB for Ethiopian operations',
  
  -- Deductions breakdown
  platform_fee REAL DEFAULT 0.00 COMMENT 'Platform processing fee (1% or min 50 ETB)',
  vat_amount REAL DEFAULT 0.00 COMMENT '15% VAT on platform fee',
  tax_amount REAL DEFAULT 0.00 COMMENT 'Withholding tax if applicable',
  other_deductions REAL DEFAULT 0.00 COMMENT 'Other fees/charges',
  
  -- Net amount (MANUALLY CALCULATED - no GENERATED to avoid conflicts)
  net_amount REAL NOT NULL COMMENT 'Net amount: amount - all deductions',
  
  -- Ethiopian banking details
  bank_name TEXT NOT NULL COMMENT 'Ethiopian bank selection',
  ...
  -- Workflow status
  status TEXT DEFAULT 'draft' COMMENT 'Current payout status',
  
  -- Request information
  requested_at TEXT DEFAULT CURRENT_TEXT COMMENT 'When payout was requested',
  requested_by INTEGEREGER NOT NULL COMMENT 'Who requested (user_id)',
  
  -- Approval information
  approved_at TEXT NULL COMMENT 'When approved',
  approved_by INTEGEREGER NULL COMMENT 'Who approved (user_id)',
  approval_notes TEXT COMMENT 'Approver comments',
  
  -- Rejection information
  rejected_at TEXT NULL COMMENT 'When rejected',
  rejected_by INTEGEREGER NULL COMMENT 'Who rejected (user_id)',
  rejection_reason VARCHAR(500) NULL COMMENT 'Reason for rejection',
  
  -- Processing method
  processing_method TEXT DEFAULT 'cbe_online' COMMENT 'How payout is processed',
  ...
  commission_total REAL DEFAULT 0.00 COMMENT 'Total commissions included',
  payment_count INTEGER DEFAULT 0 COMMENT 'Number of payments included',
  
  -- Timeline
  processed_at TEXT NULL COMMENT 'When bank processing started',
  completed_at TEXT NULL COMMENT 'When payout completed',
  failed_at TEXT NULL COMMENT 'When payout failed',
  ...
  -- Soft delete
  deleted_at TEXT NULL COMMENT 'Soft delete marker',
  
  -- Foreign Keys (strict INTEGERegrity)
  ...
  INDEX idx_payout_reference (payout_reference), -- INDEX converted separately (organizer_id), -- INDEX converted separately (status), -- INDEX converted separately (requested_at), -- INDEX converted separately (period_start_date, period_end_date), -- INDEX converted separately (approved_by), -- INDEX converted separately (completed_at), -- INDEX converted separately (deleted_at),
  
  -- Business logic constraINTEGERs (VALIDATION MOVED TO PROCEDURES)
  CONSTRAINTEGER chk_amount_positive CHECK (amount > 0),
  CONSTRAINTEGER chk_net_amount_positive CHECK (net_amount > 0),
  CONSTRAINTEGER chk_period_dates CHECK (period_end_date >= period_start_date),
  CONSTRAINTEGER chk_fees_non_negative CHECK (
    platform_fee >= 0 AND 
    vat_amount >= 0 AND 
    tax_amount >= 0 AND 
    other_deductions >= 0
...
BEGIN
  ...
  DECLARE v_organizer_exists INTEGER DEFAULT FALSE;
  ...
  IF NEW.user_id IS NULL THEN
    SELECT user_id INTEGERO NEW.user_id
    FROM organizers
  ...
  SELECT 1 INTEGERO v_organizer_exists
  FROM organizers
  WHERE id = NEW.organizer_id
    AND status = 'approved';
  ...
  IF NEW.period_start_date IS NULL THEN
    SET NEW.period_start_date = DATE_SUB(CURDATE(), INTEGERERVAL 30 DAY);
  END IF;
...
BEGIN
  ...
  -- Handle status transitions (LIGHTWEIGHT - TEXTs only)
...
BEGIN
  ...
  DECLARE v_minimum_payout REAL DEFAULT 500.00;
  DECLARE v_available_balance REAL;
  DECLARE v_platform_fee REAL;
  DECLARE v_vat_amount REAL;
  DECLARE v_net_amount REAL;
  DECLARE v_commission_ids JSON;
  DECLARE v_payment_ids JSON;
  DECLARE v_commission_count INTEGER;
  ...
  INSERT INTEGERO payouts (
    ...
    p_bank_branch,
    DATE_SUB(CURDATE(), INTEGERERVAL 30 DAY),
    CURDATE(),
    'pending_approval',
    p_requested_by
  );
  ...
  SELECT payout_reference INTEGERO p_payout_reference
  FROM payouts
...
CREATE PROCEDURE sp_approve_payout_et(
  IN p_payout_id INTEGEREGER,
  IN p_approved_by INTEGEREGER,
  IN p_notes TEXT,
...
BEGIN
  ...
  DECLARE v_commission_total REAL;
  DECLARE v_payment_count INTEGER;
  DECLARE v_commission_ids JSON;
  DECLARE v_payout_amount REAL;
...
-- Procedure 3: Process payout with bank INTEGERegration
...
BEGIN
  ...
  DECLARE v_transaction_log_id INTEGEREGER;
...
CREATE PROCEDURE sp_get_organizer_payout_summary_et(
  IN p_organizer_id INTEGEREGER,
  OUT p_available_balance REAL,
  OUT p_pending_payouts REAL,
  OUT p_total_paid REAL,
  OUT p_next_payout_date DATE
)
BEGIN
  -- Available balance (released commissions)
  SELECT COALESCE(SUM(organizer_amount), 0)
  INTEGERO p_available_balance
  FROM commissions
