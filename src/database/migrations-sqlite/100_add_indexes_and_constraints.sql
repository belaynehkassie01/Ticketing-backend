...
-- Original file: 100_add_indexes_and_constraints.sql
-- Migration: 100_add_indexes_and_constraINTEGERs.sql (FIXED - PRODUCTION SAFE)
-- Purpose: Final database optimization - add cross-table indexes and business constraINTEGERs
...
-- Audit Logs: Action type + TEXT for compliance queries
CREATE INDEX idx_audit_action_TEXT 
ON audit_logs (action_type, action_TEXT);

-- Activity Logs: User + TEXT for behavior analysis
CREATE INDEX idx_activity_user_TEXT 
ON activity_logs (user_id, activity_TEXT);
...
-- ============================================
-- PART 4: ADD UNIQUE CONSTRAINTEGERS (Business Rules)
-- ============================================

-- Ensure unique phone per user (Ethiopian format validated in application layer)
ALTER TABLE users
ADD CONSTRAINTEGER uq_users_phone UNIQUE (phone);

-- Ensure unique email per user (if provided)
ALTER TABLE users
ADD CONSTRAINTEGER uq_users_email UNIQUE (email);

-- Ensure unique TIN per organizer
ALTER TABLE organizers
ADD CONSTRAINTEGER uq_organizers_tin UNIQUE (tin_number);

-- Ensure unique business license per organizer
ALTER TABLE organizers
ADD CONSTRAINTEGER uq_organizers_license UNIQUE (business_license);

-- Ensure unique VAT number per organizer (if provided)
ALTER TABLE organizers
ADD CONSTRAINTEGER uq_organizers_vat UNIQUE (vat_number);

-- Ensure unique QR hash per entity
ALTER TABLE qr_codes
ADD CONSTRAINTEGER uq_qr_codes_hash UNIQUE (qr_hash);

-- Ensure unique ticket number
ALTER TABLE individual_tickets
ADD CONSTRAINTEGER uq_tickets_number UNIQUE (ticket_number);

-- Ensure unique payment reference
ALTER TABLE payments
ADD CONSTRAINTEGER uq_payments_reference UNIQUE (payment_reference);

-- Ensure unique payout reference
ALTER TABLE payouts
ADD CONSTRAINTEGER uq_payouts_reference UNIQUE (payout_reference);

-- Ensure unique tax code
ALTER TABLE taxes
ADD CONSTRAINTEGER uq_taxes_code UNIQUE (tax_code);

-- Ensure unique dispute code
ALTER TABLE disputes
ADD CONSTRAINTEGER uq_disputes_code UNIQUE (dispute_code);

-- Ensure unique thread code
ALTER TABLE message_threads
ADD CONSTRAINTEGER uq_threads_code UNIQUE (thread_code);

-- Ensure unique notification code
ALTER TABLE notifications
ADD CONSTRAINTEGER uq_notifications_code UNIQUE (notification_code);

-- ============================================
-- PART 5: ADD SAFE CHECK CONSTRAINTEGERS (Business Logic)
-- ============================================

-- Users: Basic phone length validation (detailed validation in application)
ALTER TABLE users
ADD CONSTRAINTEGER chk_users_phone_length 
CHECK (CHAR_LENGTH(phone) BETWEEN 10 AND 15);

-- Organizers: Commission rate validation
ALTER TABLE organizers
ADD CONSTRAINTEGER chk_organizers_commission_range 
CHECK (commission_rate BETWEEN 0 AND 100);

ALTER TABLE organizers
ADD CONSTRAINTEGER chk_organizers_custom_commission 
CHECK (custom_commission_rate IS NULL OR custom_commission_rate BETWEEN 0 AND 100);

-- Events: Date and ticket validation
ALTER TABLE events
ADD CONSTRAINTEGER chk_events_dates 
CHECK (end_date > start_date);

ALTER TABLE events
ADD CONSTRAINTEGER chk_events_tickets 
CHECK (tickets_sold <= total_tickets);

ALTER TABLE events
ADD CONSTRAINTEGER chk_events_vat_rate 
CHECK (vat_rate >= 0 AND vat_rate <= 100);

-- Ticket Types: Quantity and price validation
ALTER TABLE ticket_types
ADD CONSTRAINTEGER chk_ticket_types_quantity 
CHECK (quantity >= 0);

ALTER TABLE ticket_types
ADD CONSTRAINTEGER chk_ticket_types_sold_count 
CHECK (sold_count >= 0 AND sold_count <= quantity);

ALTER TABLE ticket_types
ADD CONSTRAINTEGER chk_ticket_types_price 
CHECK (price >= 0);

-- Payments: Amount and commission validation
ALTER TABLE payments
ADD CONSTRAINTEGER chk_payments_amount 
CHECK (amount > 0);

ALTER TABLE payments
ADD CONSTRAINTEGER chk_payments_commission 
CHECK (platform_commission >= 0 AND organizer_earning >= 0);

ALTER TABLE payments
ADD CONSTRAINTEGER chk_payments_vat 
CHECK (vat_amount >= 0);

-- Taxes: Rate validation
ALTER TABLE taxes
ADD CONSTRAINTEGER chk_taxes_rate_range 
CHECK (rate >= 0 AND rate <= 100);

-- Financial Reports: Period validation
ALTER TABLE financial_reports
ADD CONSTRAINTEGER chk_financial_reports_period 
CHECK (period_end >= period_start);

-- Reservations: Expiry validation
ALTER TABLE reservations
ADD CONSTRAINTEGER chk_reservations_expiry 
CHECK (expires_at > created_at);

-- Disputes: Amount validation
ALTER TABLE disputes
ADD CONSTRAINTEGER chk_disputes_resolution_amount 
CHECK (resolution_amount IS NULL OR resolution_amount >= 0);

-- Ethiopian Holidays: Date validation
ALTER TABLE ethiopian_holidays
ADD CONSTRAINTEGER chk_holidays_dates 
CHECK (end_date >= start_date);
...
ON activity_logs (activity_TEXT DESC, activity_type);
...
-- ============================================
-- PART 8: DATA QUALITY AND MAINTEGERENANCE INDEXES
-- ============================================
...
-- 4. SMS delivery with Ethio Telecom INTEGERegration